...
$datas = $bdd->prepare("SELECT * FROM `messages` WHERE ((`id_receiver`=? AND `id_sender`=?) OR (`id_receiver`=? AND `id_sender`=?)) AND (`id_receiver`=? AND `view`=?)  ORDER BY `id` ASC");
$datas->execute(array($id_other_user,$me,$me,$id_other_user,$me,0));
$result=array();
